...
export function RegularScheduleForm({ clientId, slot, returnTo }: { clientId: string; slot?: RegularScheduleSlot; returnTo: string }) {
  ...
  return <section className="form-page" aria-labelledby="slot-form-title">
    ...
    <form action={action} className="client-form">
      ...
      <div className="form-actions"><Link className="button button-secondary" href={returnTo}>Cancel</Link><button className="button button-primary" type="submit">Save regular time</button></div>
    </form>
  </section>;
}
