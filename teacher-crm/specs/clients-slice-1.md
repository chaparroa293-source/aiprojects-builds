...
| -------------------------- | --------------------------------- | --- | ------------------------------------------------------------------------------------------------ |
| Name                       | text                              | yes | The client — a student, usually a child                                                          |
| Status                     | enum: Prospect / Current / Former | yes | Default on create: Prospect                                                                      |
| Contact Name               | text                              | no  | Who the phone number belongs to (often a parent) — may equal Name for older/independent students |
| Phone                      | text                              | no  | Free-form (no format validation in V1) — doubles as WhatsApp                                     |
| Notes                      | long text                         | no  | Free text, no structure                                                                          |

No email field. No separate "preferred contact method." No placeholder sections for future Classes/Payments data.

## Create / Edit

- Single form used for both Add and Edit (same field set).
- Only **Name** is required to save. All other fields optional.
...
- Shows: Name, Status, Contact Name, Phone, Notes (truncated to ~40 chars with ellipsis).
- Search: matches against Name only (not Contact Name, not Notes) — keep search scope narrow and predictable for V1.
...
- Displays: Name, Status (inline editable dropdown), Contact Name, Phone, Notes.
...
1. User can add a client with only a Name and no other fields, and it appears in the list.
...
5. Searching by name (partial, case-insensitive) filters the list correctly.
