...
The shell uses a persistent sidebar on desktop and a compact navigation strip on narrow screens. Every active page follows eyebrow → serif title → short explanation → primary action → operational surface. The Clients surface adds controls, a readable table, and a detail/form panel; Client Detail uses restrained Datos/Agenda/Sesiones/Pagos tabs and becomes sequential on narrow screens. Agenda and Payments stay client-scoped; neither is a global surface.
