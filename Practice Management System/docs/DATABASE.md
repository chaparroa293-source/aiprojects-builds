...
`practice_management.sessions` stores a direct required `client_id` foreign key to `clients`, required `fecha` and `hora_inicio`, optional `duracion_minutos` (when supplied it must be positive), required `estado` (`programada`, `completada`, or `cancelada`), optional `notas`, optional nullable `payment_id`, and timestamps. `payment_id` references `practice_management.payments(id)` with `ON DELETE SET NULL`; the `sessions_payment_client_check` trigger rejects a linked Payment belonging to another Client. `sessions_client_id_idx` supports client-history reads, `sessions_payment_id_idx` supports reverse Payment reads, and `sessions_set_updated_at` reuses the scoped timestamp function. `sessions.payment_id` is the sole relationship source of truth; Payment-linked Session counts are derived, not stored.
